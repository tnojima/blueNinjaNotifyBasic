class AirDataWindow extends PApplet {
  PApplet parent;

  AirDataWindow(PApplet _parent) {
    super();
    // set parent
    this.parent = _parent;
    //// init window
...
float baseTemp=28;  /// Base value for sensor data(Temparature in degC)
  ...
  float[] sensors_min={baseTemp-tempRange/2.0, basePress-pressRange/2.0};
  ...
  void draw() {
    ...
    for (int i = 0; i < NUM; i++) {
      ...
      float ty = map(airData[i], sensors_min[i], sensors_max[i], height/NUM, 0)+height/NUM*i;

      //Draw data labels
      fill(0);
      text(label[i], 0, height/NUM*(i+0.5));
      text(airData[i], 30, height/NUM*(i+0.5));
