class MagDataWindow extends PApplet {
  PApplet parent;
  
  MagDataWindow(PApplet _parent) {
    super();
    // set parent
    this.parent = _parent;
    //// init window
...
void settings() {
  ...
  int NUM=3;
  String[] label={"MGX", "MGY", "MGZ"};
  float baseMG=0;              /// Base value for sensor data(Magnet Value)
  float MGRange=1000;  /// Sensor data range

  /// the size of this min/max array  must be the same as "NUM"
  float[] sensors_max={baseMG+MGRange/2.0, baseMG+MGRange/2.0, baseMG+MGRange/2.0};
  float[] sensors_min={baseMG-MGRange/2.0, baseMG-MGRange/2.0, baseMG-MGRange/2.0};
  ...
  void draw() {
    ...
    for (int i = 0; i < NUM; i++) {
      ...
      float ty = map(mgData[i], sensors_min[i], sensors_max[i], height/NUM, 0)+height/NUM*i;

      //Draw data labels
      fill(0);
      text(label[i], 0, height/NUM*(i+0.5));
      text(mgData[i], 30, height/NUM*(i+0.5));
